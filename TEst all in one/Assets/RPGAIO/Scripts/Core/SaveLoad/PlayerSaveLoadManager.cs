...
namespace LogicSpawn.RPGMaker.Core
{
    public class PlayerSaveLoadManager
    {
        ...
        public bool Load(string savePathOrPPrefKey)
        {
            ...
            if(Application.loadedLevelName != loadedPlayerSave.CurrentScene)
            {
            ...
            //todo: check this
            var classDef = Rm_RPGHandler.Instance.Player.ClassDefinitions.FirstOrDefault(c => c.ID == loadedPlayerSave.Character.PlayerClassID);
            if(classDef == null)
            {
                Debug.Log("Class not found for character you're trying to load.");
                RPG.LoadLevel(0);
                return false;
            }
            ...
            {
                Debug.LogError("Did not find spawn position");
            }
...
#else
            var jsonFromEncode = PlayerPrefs.GetString(lastSavePath, "");
#endif
