...
public class TargetLockHandler : MonoBehaviour
{
    ...
    private const float BasePlaneScale = 0.15f;
    ...
    void Start()
    {
        ...
        if(ShowWithTexture)
        {
            ...
            TargetLockObject.transform.localScale = new Vector3(BasePlaneScale, 1, BasePlaneScale);
            TargetLockObject.transform.localScale *= (radius * 2);
        }
        else
        {
            var t = transform.FindChild("TargetLockPrefab");
            if (t == null)
            {
            ...
            case TargetLockState.Selected:
                if (ShowWithTexture)
                {
                    ...
                    if (Rm_RPGHandler.Instance.Combat.SelectedTexture.Image != null)
                        _material.SetTexture(0, Rm_RPGHandler.Instance.Combat.SelectedTexture.Image);
                }
                else
                {
            ...
            case TargetLockState.InCombat:
                if (ShowWithTexture)
                {
                    ...
                    if (Rm_RPGHandler.Instance.Combat.SelectedCombatTexture.Image != null)
                        _material.SetTexture(0, Rm_RPGHandler.Instance.Combat.SelectedCombatTexture.Image);
                }
                else
                {
