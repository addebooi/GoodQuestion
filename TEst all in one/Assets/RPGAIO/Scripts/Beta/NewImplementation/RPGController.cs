...
namespace Assets.Scripts.Beta.NewImplementation
{
    public class RPGController : MonoBehaviour, IRPGController
    {
        ...
        private const float AggroRadius = 25.0f;
        ...
        public void SetPlayerControl(GameObject combatant)
        {
            ...
            _rpgCamera.SwitchToCharacter(combatant.transform);
            _playerDirWorld = Vector3.zero;
            ForceStopHandlingActions();
        }
                    ...
                    case RPGActionType.AutoAttack:
                        {
                            ...
                            if (Rm_RPGHandler.Instance.Combat.TargetStyle == TargetStyle.ManualTarget && targetPosition == Vector3.zero)
                            {
                                var pos = targetToDmg.transform.Center();
                                targetPosition = targetPosition == Vector3.zero ? pos : targetPosition;
                            }
                            ...
                            if (attackStyle == AttackStyle.Ranged)
                            {
                                ...
                                ? Rm_RPGHandler.Instance.Player.ClassDefinitions.First(c => c.ID == ((PlayerCharacter)Character).PlayerClassID).AutoAttackPrefabPath
        ...
        private LegacyAnimation GetLegacyAnimation()
        {
            switch (Character.CharacterType)
            {
                case CharacterType.Player:
                    ...
                    if (playerChar != null)
                    {
                        return Rm_RPGHandler.Instance.Player.ClassDefinitions.First(c => c.ID == playerChar.PlayerClassID).LegacyAnimations;
                    }
                    break;
        ...
        private void GetInput()
        {
            ...
            _moveForward = RPG.Input.GetAxis(RPG.Input.VerticalAxis);
        ...
        private void GetClickInput()
        {
            ...
            if (_showingCastArea && RPG.Input.GetKey(RPG.Input.ConfirmCast))
            {
                ...
                RPGCombat.UseSkill(_skillToCast, GetPositionAtMousePosition(), rotation);
                EndCastArea();
            }
        ...
        private void GetAIActions()
        {
            ...
            else if (Vector3.Distance(transform.position, SpawnPosition) > TooFarFromSpawnDistance + NavMeshAgent.radius) //If we are too far away from spawnPos
            {
                ForceStopHandlingActions();
            ...
            else if (combatChar.RetreatsWhenLow && !_alreadyRetreated && Character.VitalHandler.Health.CurrentValue < (Character.VitalHandler.Health.MaxValue * 0.1f)) //If we're about to die retreat //todo: check if this char retreats
            {
            ...
            else if (_state == RPGControllerState.Follow)   
            {
                ...
                if (Vector3.Distance(transform.position, targetToFollow.transform.position) < 1.5f + NavMeshAgent.radius * transform.localScale.x)
                {
                    RPGAnimation.CrossfadeAnimation(LegacyAnimation.IdleAnim);
                }
                else
                {
                    queue.Add(RPGActionFactory.MoveToPosition(targetToFollow, 1.5f + NavMeshAgent.radius * transform.localScale.x).WithAnimation(LegacyAnimation.RunAnim));
                }
            }
        ...
        private void Start()
        {
            ...
            _rpgCamera.cameraMode = cameraMode;
            Interacting = false;
        ...
        private void Update()
        {
            ...
            if (!CharacterMono.Initialised) return;
            ...
            if (stopwatch.ElapsedMilliseconds > 10)
                Debug.Log("2 Spike!" + stopwatch.ElapsedMilliseconds.ToString());
            ...
            //todo:remove testing
            {
                if (IsPlayerControlled)
                {
                    ...
                    {
                        AddImpact(Direction.Up, 12);
                    }
                    if (Input.GetKeyDown(KeyCode.F3))
                    {
                        AddImpact(Direction.Back, 20);
                    }

                    if (Input.GetKeyDown(KeyCode.F4))
                    {
                        var sound = new AudioContainer { AudioPath = "Audio/gun" };
                        BeginActionQueue(RPGActionQueue.Create(RPGActionFactory.PlaySound(sound, AudioType.SoundFX, 10)));
                    }
