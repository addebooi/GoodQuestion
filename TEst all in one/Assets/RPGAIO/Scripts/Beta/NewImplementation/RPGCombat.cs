...
namespace Assets.Scripts.Beta.NewImplementation
{
    public class RPGCombat : MonoBehaviour, IRPGCombat
    {
        ...
        private void UseSkill(Skill originalSkill, Vector3? nullableTargetPos, BaseCharacterMono target, Quaternion rotation = default(Quaternion))
        {
            ...
            {
                animations = skillToUse.LegacyAnimationToUse(player.PlayerClassID);
            }
            else
            {
            ...
            if (skillToUse.SkillType == SkillType.Melee)
            {
                ...
                meleeSkillDefinition = Character.CharacterType == CharacterType.Player
                                           ? meleeSkill.MeleeAnimations.FirstOrDefault(m => m.ClassID == player.PlayerClassID)
            ...
            //Casting:
            if (!string.IsNullOrEmpty(castingAnimName) && castingTime > 0)
            {
            ...
            float postImpactTime)
        {
            ...
            if (skillToUse.SkillType == SkillType.Melee)
            {
                if (meleeSkillDefinition != null)
                {
                    ...
                    castLength = !string.IsNullOrEmpty(castAnimName) ? _controller.Animation[castAnimName].length : 0.5f;
                }
            }
            ...
            {
                if (!string.IsNullOrEmpty(castAnimName) && castTime > 0)
                {
            ...
            else
            {
                ...
                else if (skillMoveType == SkillMovementType.JumpTo)
                {
                    ...
                    var jumpAnimLength = !string.IsNullOrEmpty(jumpToAnimName) ? _controller.Animation[jumpToAnimName].length : 0.5f;
            ...
            if (skillMoveType != SkillMovementType.StayInPlace)
            {
                if (!string.IsNullOrEmpty(landAnimName))
                {
        ...
        public void Attack(BaseCharacterMono target, Vector3 position = default(Vector3))
        {
            ...
            if(attackStyle == AttackStyle.Melee)
            {
                ...
                if (Character.CharacterType == CharacterType.Player)
                {
                    var classDef = Rm_RPGHandler.Instance.Player.ClassDefinitions.First(c => c.ID == ((PlayerCharacter)Character).PlayerClassID);
            ...
            if(outcome != null)
            {
                ...
                if (!outcome.KilledTarget)
                {
                    ...
                    if(target.Controller.Animation != null && !target.Controller.HandlingActions)
                    {
                        ...
                        if(!string.IsNullOrEmpty(getHitAnim.Animation) && target.Controller.IsGrounded)
                        {
