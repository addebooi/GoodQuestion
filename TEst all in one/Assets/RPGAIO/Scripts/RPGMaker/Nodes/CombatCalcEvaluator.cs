...
namespace LogicSpawn.RPGMaker
{
    public class CombatCalcEvaluator
    {
        ...
        {
            if(attacker.CharacterType == CharacterType.Player || defender.CharacterType == CharacterType.Player)
            {
            ...
            {
                return new DamageOutcome(nodeChain.Damage, nodeChain.Damage.IsCritical ? AttackOutcome.Critical : AttackOutcome.Success);
            }
        }
        ...
        public static Damage EvaluateDamageDealt(BaseCharacter character)
        {
            ...
            if (dmgHasVariance)
            {
                nodeChain = new NodeChain(combatCalcTree, typeof(CombatStartNode), "MIN_Physical") { Combatant = combatant, IntValue = damage.MinDamage };
                while (!nodeChain.Done)
                {
                    nodeChain.Evaluate();
                } 
