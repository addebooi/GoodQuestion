            ...
            if (selectedInfo != null)
            {
                ...
                switch (selectedInfo.VariableType)
                {
                    case Rmh_CustomVariableType.Float:
                        selectedInfo.FloatValue = RPGMakerGUI.FloatField("Equals Value:", selectedInfo.FloatValue);
                        break;
                    case Rmh_CustomVariableType.Int:
                        selectedInfo.IntValue = RPGMakerGUI.IntField("Equals Value:", selectedInfo.IntValue);
                        break;
                    case Rmh_CustomVariableType.String:
                        selectedInfo.StringValue = RPGMakerGUI.TextField("Equals Value:", selectedInfo.StringValue);
                        break;
                    case Rmh_CustomVariableType.Bool:
                        selectedBooleanResult = selectedInfo.BoolValue ? 0 : 1;
                        selectedBooleanResult = RPGMakerGUI.Popup("Return:", selectedBooleanResult, new[] { "True", "False" });
