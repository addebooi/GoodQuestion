    ...
    public static class Rme_Main_Items
    {
        ...
        public static void Options(Rect fullArea, Rect leftArea, Rect mainArea)
        {
            ...
            if (RPGMakerGUI.Foldout(ref showItemOptions, "Options"))
            {
                ...
                Items.DamageHasVariance = RPGMakerGUI.Toggle("Damage has variance?", Items.DamageHasVariance);
...
//                    var allItems = Rm_RPGHandler.Instance.Repositories.Items.AllItems;
        ...
        public static void LootTables(Rect fullArea, Rect leftArea, Rect mainArea)
        {
            ...
            if (selectedLootTable != null)
            {
                ...
                var lootTableResult = RPGMakerGUI.FoldoutToolBar(ref showLootTableItems, "Loot Table Items", new []{"+Item", "+Gold", "+Empty"});
                if(showLootTableItems)
                {
                    ...
                    for (int index = 0; index < selectedLootTable.LootTableItems.Count; index++)
                    {
                        ...
                        {
                            var stillExists =
                                allItems.FirstOrDefault(a => a.ID == selectedLootTableItem.ItemID);
                        ...
                        else if (allItems.Count > 0)
                        {
                            ...
                            selectedLootTableItem.ItemID = allItems[selectedLootTableItemIndex].ID;

                            //todo: add to 100 : selectedLootTableItem.ChanceForItem
                            var foundItem = allItems[selectedLootTableItemIndex];
                    ...
                    {
                        selectedLootTable.LootTableItems.Add(new Rm_LootTableItem());
                        GUI.FocusControl("");
                    }
