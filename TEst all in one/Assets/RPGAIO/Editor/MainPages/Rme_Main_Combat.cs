            ...
            if (selectedSkill != null)
            {
                ...
                if (selectedSkill.SkillType == SkillType.Melee)
                {
                    ...
                    foreach (var d in Rm_RPGHandler.Instance.Player.ClassDefinitions)
                    {
                        ...
                        if (tier == null)
                        {
                            if (selectedSkill.ClassIDs.Contains(d.ID) || selectedSkill.AllClasses)
                            {
                    ...
                    for (int index = 0; index < meleeSkill.MeleeAnimations.Count; index++)
                    {
                        ...
                        if (stillExists == null || (!selectedSkill.ClassIDs.Contains(v.ClassID) && !selectedSkill.AllClasses))
                        {
                ...
                if (RPGMakerGUI.Foldout(ref showSkillDetails, "Skill Details"))
                {
                    ...
                    if (!selectedSkill.AllClasses)
                    {
                        ...
                        if (showSkillClasses)
                        {
                            ...
                            for (int i = 0; i < selectedSkill.ClassIDs.Count; i++)
                            {
                                ...
                                RPGMakerGUI.PopupID<Rm_ClassDefinition>("", ref refString);
                    ...
                    if (GUILayout.Button("Open Custom Damage Scaling Window", "genericButton", GUILayout.MaxHeight(30)))
                    {
                        ...
                        if (existingTree == null)
                        {
                            ...
                            var node = new CombatStartNode("Physical Damage", "Start node for dealt physical",
                               "Physical Damage represents weapon damage, or base combatant damage. You can then add additional nodes to add onto this value. Example: Add 10x Strength") { Identifier = "Physical" };
                            node.OnCreate();
                            existingTree.AddNode(node, new Vector2(25, 87.5f), 0);
                ...
                if (RPGMakerGUI.Foldout(ref showAnimationInfo, "Player Animations"))
                {
                    ...
                    if(selectedSkill.SkillType != SkillType.Melee)
                    {
                        for (int index = 0; index < animList.Count; index++)
                        {
                            ...
                            var className = Rm_RPGHandler.Instance.Player.ClassDefinitions.First(c => c.ID == anim.ClassID).Name;
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < meleeAnims.Count; i++)
                        {
                            ...
                            var className = Rm_RPGHandler.Instance.Player.ClassDefinitions.First(c => c.ID == meleeAnims[i].ClassID).Name;
                            RPGMakerGUI.SubTitle(className + " Animations:");
